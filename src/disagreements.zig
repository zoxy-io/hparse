...
pub const requests = [_]Entry{
    ...
    .{ .hparse = true, .pico = true, .llhttp = false, .why = "raw non-ASCII in the request-target; strictly it must be percent-encoded, but hparse hands the target back as an opaque slice and never decodes it, so a high byte cannot end the target early or introduce a header. Revisit if hparse ever interprets the path.", .input = "GET /\xce\xb4\xc2\xb6/\xce\xb4t/pope?q=1#narf HTTP/1.1\r\nHost: github.com\r\n\r\n" },
};
